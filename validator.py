...
class ResponseValidator:
    ...
    def _extract_config(self) -> Optional[Dict]:
        ...
        # This regex pattern finds a block starting with ```yaml or ```
        # and captures everything until the closing ```.
        # re.DOTALL is crucial because it allows '.' to match newline characters.
        # re.IGNORECASE handles both 'yaml' and 'YAML'.
        yaml_pattern = r"```(?:yaml)?\n(.*?)\n```"
        ...
        if match:
            ...
            try:
                # Now, attempt to parse the clean string as YAML
                return yaml.load(config_string, Loader=yaml.FullLoader)
            except yaml.YAMLError as e:
                # The string inside the block was not valid YAML.
                self.errors.append(f"Failed to parse YAML content within the code block. Error: {e}")
                return None     
       
       
        # if "toml:" in response_lower:
        #     try:
